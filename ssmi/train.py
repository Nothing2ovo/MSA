...
from data_loader import load_mosi_from_pkl
...
def resolve_data_file() -> str:
    candidates = [
        os.environ.get("MOSI_PKL", "").strip(),
        os.environ.get("SSMI_DATA", "").strip(),
        DEFAULT_DATA_FILE,
        os.path.join(BASE_DIR, "aligned_50.pkl"),
        os.path.join(SCRIPT_DIR, "aligned_50.pkl"),
    ]
    checked = []
    for path in candidates:
        ...
        if os.path.exists(path):
            return path
    raise FileNotFoundError("Could not find MOSI aligned_50.pkl. Searched:\n" + "\n".join(checked))
...
def main() -> None:
    ...
    train_dataset, valid_dataset, test_dataset, meta = load_mosi_from_pkl(data_file)
    print("dataset: MOSI")
